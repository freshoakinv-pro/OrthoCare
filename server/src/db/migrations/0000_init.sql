...
CREATE TABLE `prom_link_tokens` (
	...
	`created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
	CONSTRAINT `prom_link_tokens_id` PRIMARY KEY(`id`),
	CONSTRAINT `prom_link_tokens_jti_uidx` UNIQUE(`jti`)
);
--> statement-breakpoint
...
ALTER TABLE `prom_link_tokens` ADD CONSTRAINT `prom_link_tokens_patient_id_patients_id_fk` FOREIGN KEY (`patient_id`) REFERENCES `patients`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `prom_link_tokens` ADD CONSTRAINT `prom_link_tokens_prom_type_id_prom_types_id_fk` FOREIGN KEY (`prom_type_id`) REFERENCES `prom_types`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `prom_link_tokens` ADD CONSTRAINT `prom_link_tokens_episode_id_clinical_episodes_id_fk` FOREIGN KEY (`episode_id`) REFERENCES `clinical_episodes`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `prom_link_tokens` ADD CONSTRAINT `prom_link_tokens_clinic_id_clinics_id_fk` FOREIGN KEY (`clinic_id`) REFERENCES `clinics`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `prom_link_tokens_patient_id_idx` ON `prom_link_tokens` (`patient_id`);--> statement-breakpoint
CREATE INDEX `prom_link_tokens_clinic_id_idx` ON `prom_link_tokens` (`clinic_id`);--> statement-breakpoint
